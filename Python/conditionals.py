...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    nuetron_x_temperature = neutrons_emitted * temperature
    if temperature < 800.0 and neutrons_emitted > 500.0 and nuetron_x_temperature < 500000.0:
        balanced = True
    else:
        balanced = False
    return balanced
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if reactor < (0.9*threshold):
        status_code = "LOW"
    elif reactor >= 0.9*threshold and reactor < 1.1*threshold:
        status_code = "NORMAL"
    else:
        status_code = "DANGER"
    print(status_code)
